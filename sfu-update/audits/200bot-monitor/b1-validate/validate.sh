...
log "ROOM=${ROOM}: 20 presenters + slow-join 50/25s to 400 (the failing shape)"
for i in $(seq -w 1 20); do ap "b1val-snd${i}" "s${i}" 1 0 460; done
for i in 1 2 3 4 5 6 7 8; do ap "b1val-l${i}" "l${i}" 0 50 $((400-i*25)); log "  wave $i (+50 => $((i*50))) cpu=$(cpu_m)m"; sleep 25; done
